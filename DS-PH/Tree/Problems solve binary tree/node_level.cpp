...
int level(node* root, int x)
{
    ...
    while (!q.empty())
    {
        // 1st rule: line theke ber kora
        pair<node*, int> pr = q.front();
        q.pop();

        // pair theke node and level alada korte hobe, 
        node* node = pr.first;
        int level = pr.second;

        // 2nd rule: jabotio kaj
        if(node->val == x)
        {
            return level;
        }

        // 3rd rule: childreen gulo ber kora
...
int main()
{
     ...
     cout<<"\nThe maximum height of tree is: "<<level(root,x);
