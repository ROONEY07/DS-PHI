...
int main()
{
    ...
    int maxDepth = max_depth(root); // Calculate max depth of the tree
    int totalNodes = total_nodes(root); // Calculate total number of nodes
    ...
    int TotalNumberNodes = pow(2,maxDepth) - 1;
    
    if(totalNodes == TotalNumberNodes)
    {
        cout<<"YES"<<endl;
    }
    else
